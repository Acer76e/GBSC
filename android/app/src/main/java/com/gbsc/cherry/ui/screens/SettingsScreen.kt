...
fun SettingsScreen(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        Text(
            "Choose where the card sits and which info it shows.",
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            CardPosition.entries.forEach { pos ->
                FilterChip(
                    ...
                    label = { Text(pos.name.lowercase().replaceFirstChar { it.uppercase() }) },
                )
            }
        }
        ...
        ToggleRow("Live trip profit", c.showProfit) { v -> update { it.copy(showProfit = v) } }

        Spacer(Modifier.height(16.dp))
        Text("Notifications", fontWeight = FontWeight.Bold)
        Spacer(Modifier.height(4.dp))
        ToggleRow("Offer notification with map shortcuts", c.notificationEnabled) { v ->
